    ...
    <div class="py-8 space-y-8">
        ...
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('leaves.create') }}" 
                class="inline-flex items-center px-6 py-3 bg-blue-600 text-white font-medium rounded-lg shadow hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-offset-gray-800">
                + Add Leave
            </a>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
            @if ($leavesuser->count() > 0)
                <table class="w-full text-sm text-gray-700 dark:text-gray-300">
                    <thead class="bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200">
                        <tr>
                            <th class="px-6 py-4 font-semibold text-left">Leave ID</th>
                            <th class="px-6 py-4 font-semibold text-left">Employee ID</th>
                            <th class="px-6 py-4 font-semibold text-left">Start Date</th>
                            <th class="px-6 py-4 font-semibold text-left">End Date</th>
                            <th class="px-6 py-4 font-semibold text-left">Status</th>
                            <th class="px-6 py-4 font-semibold text-left">Remarks</th>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex justify-center space-x-4">
                                        <a href="{{ route('leaveuser.edit', ['id' => $leave->leave_id, 'employee_id' => $leave->employee_id]) }}" 
                                            class="text-yellow-500 hover:text-yellow-600">
                                            Edit
                                        </a>
                                        <form action="{{ route('leaveuser.destroy', ['id' => $leave->leave_id, 'employee_id' => $leave->employee_id]) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" 
        ...
        <div class="flex justify-center">
            {{ $leavesuser->links('pagination::tailwind') }}
        </div>
    </div>
</x-app-layout>
