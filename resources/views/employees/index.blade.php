...
<x-app-layout>
    ...
    <div class="py-12 bg-gradient-to-b from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-900 min-h-screen">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            @if (session('success'))
                <div
                    class="bg-green-500 text-white px-6 py-4 rounded-lg shadow-lg text-center transition-transform transform hover:scale-105">
                    {{ session('success') }}
                </div>
            @endif
            ...
            <div class="bg-white dark:bg-gray-800 shadow-xl rounded-lg overflow-hidden">
                ...
                <div class="p-8 space-y-8">
                    ...
                    <div>
                        <h4
                            class="flex items-center gap-2 text-lg font-semibold text-gray-700 dark:text-gray-200 border-b pb-2 mb-4">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-600 dark:text-gray-400">
                            <p><strong>Address Line 1:</strong> {{ $employeess->address_line_1 ?? 'Not available' }}
                            </p>
                            <p><strong>Address Line 2:</strong> {{ $employeess->address_line_2 ?? 'Not available' }}
                            </p>
                    ...
                    <div>
                        <h4
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-purple-500" viewBox="0 0 20 20"
                                fill="currentColor">
                                <path
                                    d="M10 2a8 8 0 100 16 8 8 0 000-16zm-1 11a1 1 0 11-2 0 1 1 0 012 0zm4 0a1 1 0 11-2 0 1 1 0 012 0zM9 8a1 1 0 011-1h4a1 1 0 110 2h-4a1 1 0 01-1-1z" />
                            </svg>
        ...
        <div class="p-8 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <!-- Certificates Card -->
                <div
                    class="bg-white dark:bg-gray-800 shadow-2xl rounded-lg p-6 transform transition-transform hover:scale-105 duration-300">
                    <h4 class="text-xl font-semibold text-gray-700 dark:text-gray-200 mb-4">CERTIFICATES</h4>
                    <div class="space-y-4">
                        @foreach ($employeess->certificates as $certificate)
                            <div
                                class="bg-gradient-to-r from-gray-50 via-gray-100 to-gray-200 dark:from-gray-700 dark:via-gray-800 dark:to-gray-900 p-4 rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300">
                                <h6 class="text-lg font-medium text-gray-800 dark:text-gray-200 uppercase">
                                    {{ $certificate->certificate_name }}</h6>
                                <p class="text-gray-600 dark:text-gray-400 text-sm">Issued by:
                                    {{ $certificate->issued_by }}</p>
                                <p class="text-gray-500 dark:text-gray-300 text-sm">Issued on:
                                    {{ $certificate->issue_date }}</p>
                            </div>
                        @endforeach
                        @if ($employeess->certificates->isEmpty())
                            <p class="text-gray-500 dark:text-gray-300">No certificates found.</p>
                        @endif
                    </div>
                </div>
                ...
                <div
                    class="bg-white dark:bg-gray-800 shadow-2xl rounded-lg p-6 transform transition-transform hover:scale-105 duration-300">
                    <h4 class="text-xl font-semibold text-gray-700 dark:text-gray-200 mb-4">Skills</h4>
                    @if ($employeess->skills->isNotEmpty())
                        <ul class="space-y-3">
                            @foreach ($employeess->skills as $skill)
                                <li
                                    class="flex items-center text-gray-600 dark:text-gray-400 transition-transform transform hover:scale-105 duration-300 border rounded-lg p-3">
                                    <i class="fas fa-cogs h-5 w-5 mr-3 text-gray-400 dark:text-gray-300"></i>
                                    <span class="font-medium">{{ $skill->skill_name }}</span> - <span
                                        class="text-gray-500 dark:text-gray-400">{{ $skill->proficiency_level }}</span>
                                </li>
                            @endforeach
                        </ul>
                    @else
                        <p class="text-gray-500 dark:text-gray-300">No skills found.</p>
                    @endif
                </div>
                ...
                <div
                    class="bg-white dark:bg-gray-800 shadow-2xl rounded-lg p-6 transform transition-transform hover:scale-105 duration-300">
                    <h4 class="text-xl font-semibold text-gray-700 dark:text-gray-200 mb-4">EDUCATION</h4>
                    @if ($employeess->education->isNotEmpty())
                        <div class="space-y-4">
                            @foreach ($employeess->education as $education)
                                <div
                                    class="bg-gradient-to-r from-gray-50 via-gray-100 to-gray-200 dark:from-gray-700 dark:via-gray-800 dark:to-gray-900 p-4 rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300">
                                    <h6 class="text-lg font-medium text-gray-800 dark:text-gray-200 uppercase">
                                        {{ $education->degree }} in {{ $education->field_of_study }}</h6>
                                    <p class="text-gray-600 dark:text-gray-400 text-sm">Institution:
                                        {{ $education->institution_name }}</p>
                                    <p class="text-gray-500 dark:text-gray-300 text-sm">{{ $education->start_date }} -
                                        {{ $education->end_date }}</p>
                                </div>
                            @endforeach
                        </div>
                    @else
                        <p class="text-gray-500 dark:text-gray-300">No education records found.</p>
                    @endif
                </div>
                ...
                <div
                    class="bg-white dark:bg-gray-800 shadow-2xl rounded-lg p-6 transform transition-transform hover:scale-105 duration-300">
                    <h4 class="text-xl font-semibold text-gray-700 dark:text-gray-200 mb-4">Languages</h4>
                    @if ($employeess->languages->isNotEmpty())
                        <ul class="space-y-3">
                            @foreach ($employeess->languages as $language)
                                <li
                                    class="flex items-center text-gray-600 dark:text-gray-400 transition-transform transform hover:scale-105 duration-300 border rounded-lg p-3">
                                    <i class="fas fa-language h-5 w-5 mr-3 text-gray-400 dark:text-gray-300"></i>
                                    <span class="font-medium">{{ $language->languagesetup->name }}</span>
                                    ({{ $language->proficiency_level }})
                                </li>
                            @endforeach
                        </ul>
                    @else
                        <p class="text-gray-500 dark:text-gray-300">No languages found.</p>
                    @endif
                </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function getRandomLightColor() {
                const r = Math.floor(Math.random() * 155) +
                    100; // Random value between 100 and 255 for a light color
            ...
            if (profileHeader) {
                profileHeader.style.background =
                    `linear-gradient(to right, ${randomDarkColor}, ${randomLightColor})`;
            }
        });
    </script>
</x-app-layout>
