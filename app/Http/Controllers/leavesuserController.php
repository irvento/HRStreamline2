    ...
    public function index(Request $request)
{
    ...
    // Fetch all employee IDs from employeeModel for the authenticated user
    $employeeIds = employeeModel::where('user_id', $user->id)->pluck('employee_id');

    // Check if there are any matching employee IDs
    if ($employeeIds->isEmpty()) {
        return redirect()->back()->with('error', 'No employees found for the user.');
    }

    // Build the query for leavesModel based on the employee IDs
    $query = leavesModel::whereIn('employee_id', $employeeIds);
